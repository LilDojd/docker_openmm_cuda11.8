...
if [ $(id -u) == 0 ] ; then
    ...
    # Only attempt to change the darkforest username if it exists
    if id darkforest &> /dev/null ; then
        echo "Set username to: $NB_USER"
        usermod -d /home/$NB_USER -l $NB_USER darkforest
    fi
